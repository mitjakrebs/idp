theory \<^marker>\<open>tag invisible\<close> Orderings_Ext
  imports Main
begin

instantiation \<^marker>\<open>tag invisible\<close> prod :: (linorder, linorder) linorder
begin

abbreviation \<^marker>\<open>tag invisible\<close> less_prod' where
...
definition \<^marker>\<open>tag invisible\<close> less_prod where
  "less_prod \<equiv> less_prod'"

definition \<^marker>\<open>tag invisible\<close> eq_prod where
  "eq_prod p1 p2 \<equiv>
   case p1 of (a1::'a::linorder, b1::'b::linorder) \<Rightarrow>
     case p2 of (a2::'a::linorder, b2::'b::linorder) \<Rightarrow>
       if (a1 = a2) \<and> (b1 = b2) then True else False"

definition \<^marker>\<open>tag invisible\<close> less_eq_prod where
  "less_eq_prod p1 p2 \<equiv> less_prod' p1 p2 \<or> eq_prod p1 p2"

instance \<^marker>\<open>tag invisible\<close> proof
