...
lemma \<^marker>\<open>tag invisible\<close> even_if_last:
...
lemma \<^marker>\<open>tag invisible\<close> odd_if_last:
  assumes "alt_list P Q l"
  assumes "\<forall>x\<in>set l. P x \<longleftrightarrow> \<not> Q x"
  assumes "P (last l)"
  assumes "l \<noteq> []"
...
lemma \<^marker>\<open>tag invisible\<close> alt_pathD:
...
lemma \<^marker>\<open>tag invisible\<close> alt_pathI:
...
lemma \<^marker>\<open>tag invisible\<close> alt_path_snoc_snoc:
...
lemma \<^marker>\<open>tag invisible\<close> alt_path_refl:
  assumes "v \<in> Vs G"
...
lemma \<^marker>\<open>tag invisible\<close> alt_list_append_3':
  assumes "alt_list P1 P2 l1"
...
lemma \<^marker>\<open>tag invisible\<close> alt_list_tl:
  assumes "l \<noteq> []"
...
lemma \<^marker>\<open>tag invisible\<close> alt_list_butlast:
  assumes "l \<noteq> []"
  assumes "alt_list P1 P2 l"
...
text \<open>And we can split alternating paths.\<close>
...
proof (rule alt_pathI, goal_cases)
  ...
  have "alt_list P (Not \<circ> P) (edges_of_path ?p') \<or> alt_list (Not \<circ> P) P (edges_of_path ?p')"
    using assms(1) Berge.edges_of_path_append[of p ?p']
...
proof (rule alt_pathI, goal_cases)
  ...
  proof (cases "odd (path_length (p @ [v]))")
    ...
    moreover have "odd (path_length p')"
      using assms True
      by (auto dest: alt_path_pref two_alt_pathsD)
...
lemma \<^marker>\<open>tag invisible\<close> distinct_alt_pathI:
  assumes "alt_path P Q G p u v"
  assumes "distinct p"
...
lemma \<^marker>\<open>tag invisible\<close> alt_list_decomp:
...
lemma \<^marker>\<open>tag invisible\<close> (in graph) alt_path_decomp:
  assumes "alt_path P Q G p u v"
...
lemma (in graph) distinct_alt_path_alt_path_to_distinct:
  assumes "alt_path P Q G p u v"
  assumes "\<not> (\<exists>c. path G c \<and> odd_cycle c)"
...
proof -
  ...
  next
    ...
    hence "even (path_length r)"
      by (force elim: closed_path_decompE_2 intro: even_path_length_cycleI)
...
text \<open>Finally, we define reachability via alternating paths in the natural way.\<close>
