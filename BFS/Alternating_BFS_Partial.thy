...
abbreviation (in alt_bfs) alt_bfs_partial :: "'n \<Rightarrow> 'n \<Rightarrow> 'a \<Rightarrow> 'm" where
  "alt_bfs_partial G1 G2 src \<equiv> parent (alt_loop_partial G1 G2 src (init src))"

lemma (in alt_bfs_invar_tbd) alt_loop_partial_eq_alt_loop:
...
lemma (in alt_bfs_invar_tbd) alt_bfs_partial_eq_alt_bfs:
  shows "alt_bfs_partial G1 G2 src = alt_bfs G1 G2 src"
  using alt_bfs_invar_init
  by (simp add: alt_loop_partial_eq_alt_loop)

theorem (in alt_bfs_invar_tbd) alt_bfs_partial_correct:
  shows "shortest_alt_path_Map P'' G src (alt_bfs_partial G1 G2 src)"
...
corollary (in alt_bfs) alt_bfs_correct:
  ...
  shows "shortest_alt_path_Map (\<lambda>e. e \<in> G.E G2) (G.union G1 G2) src (alt_bfs_partial G1 G2 src)"
  using assms
  by (intro alt_bfs_invar_tbd.alt_bfs_partial_correct)
