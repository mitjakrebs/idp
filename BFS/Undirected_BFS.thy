theory \<^marker>\<open>tag invisible\<close> Undirected_BFS
  imports
...
locale \<^marker>\<open>tag invisible\<close> undirected_bfs_valid_input = bfs where
...
sublocale \<^marker>\<open>tag invisible\<close> symmetric_adjacency
...
sublocale \<^marker>\<open>tag invisible\<close> bfs_valid_input
proof (standard, goal_cases)
  case 1
  show ?case
    using invar_G
...
abbreviation \<^marker>\<open>tag invisible\<close> (in bfs) undirected_bfs_valid_input' :: "'n \<Rightarrow> 'a \<Rightarrow> bool" where
...
abbreviation \<^marker>\<open>tag invisible\<close> (in bfs) is_shortest_path_Map :: "'n \<Rightarrow> 'a \<Rightarrow> 'm \<Rightarrow> bool" where
...
lemma \<^marker>\<open>tag invisible\<close> (in undirected_bfs_valid_input) dist_eq_dist:
  shows "Shortest_Path.dist (G.E G) u v = dist G u v"
  using dist_eq_dist
  by (simp add: dE_eq_dEs)

lemma \<^marker>\<open>tag invisible\<close> (in undirected_bfs_valid_input) is_shortest_path_iff_is_shortest_dpath:
...
lemma \<^marker>\<open>tag invisible\<close> (in undirected_bfs_valid_input) reachable_iff_reachable:
  shows "reachable (G.E G) u v \<longleftrightarrow> Noschinski_to_DDFS.reachable (G.dE G) u v"
  using reachable_iff_reachable
  by (simp add: dE_eq_dEs)

lemma \<^marker>\<open>tag invisible\<close> (in undirected_bfs_valid_input) undirected_bfs_correct:
...
lemma \<^marker>\<open>tag invisible\<close> (in bfs) undirected_bfs_correct:
