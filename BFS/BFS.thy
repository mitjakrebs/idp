...
definition discovered :: "'a \<Rightarrow> 'm \<Rightarrow> 'a \<Rightarrow> bool" where
  "discovered src m v \<longleftrightarrow> v = src \<or> P_lookup m v \<noteq> None"
...
definition traverse_edge :: "'a \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> ('q, 'm) state \<Rightarrow> ('q, 'm) state" where
  "traverse_edge src u v s \<equiv>
   if \<not> discovered src (parent s) v then discover u v s
   else s"
  ...
  "loop G src s =
   (if \<not> DONE s
    then let
          u = Q_head (queue s);
          q = Q_tail (queue s)
         in loop G src (fold (traverse_edge src u) (G.adjacency G u) (s\<lparr>queue := q\<rparr>))
    else s)"
...
lemma queue_traverse_edge_cong:
  shows "queue (traverse_edge src u v s) = (if \<not> discovered src (parent s) v then Q_snoc (queue s) v else queue s)"
  by (simp add: traverse_edge_def)

subsubsection \<open>@{term "Q_invar \<circ> queue"}\<close>

lemma invar_queue_traverse_edge:
  assumes "Q_invar (queue s)"
...
lemma list_queue_traverse_edge_cong:
  ...
  shows
    "Q_list (queue (traverse_edge src u v s)) =
     Q_list (queue s) @ (if \<not> discovered src (parent s) v then [v] else [])"
  using assms
...
lemma T_traverse_edge_cong:
  assumes "P_invar (parent s)"
  shows "T (parent (traverse_edge src u v s)) = T (parent s) \<union> (if \<not> discovered src (parent s) v then {(u, v)} else {})"
  using assms
...
lemma invar_queue_fold_2:
  assumes "G.invar G"
  assumes "Q_invar (queue s)"
  assumes "\<not> DONE s"
  shows "Q_invar (queue (fold G src s))"
  using assms
  by (auto intro: G.distinct_adjacency invar_tail invar_queue_fold)
...
lemma list_queue_fold_cong_aux:
  ...
  shows "filter (Not \<circ> discovered src (parent (traverse_edge src u v s))) vs = filter (Not \<circ> discovered src (parent s)) vs"
proof (rule filter_cong)
  fix w
  assume "w \<in> set vs"
  hence "discovered src (parent (traverse_edge src u v s)) w = discovered src (parent s) w"
    using assms
    by (auto simp add: discovered_def lookup_parent_traverse_edge_cong override_on_insert')
  thus "(Not \<circ> discovered src (parent (traverse_edge src u v s))) w = (Not \<circ> discovered src (parent s)) w"
    by simp
qed simp
...
lemma list_queue_fold_cong:
  ...
  shows
    "Q_list (queue (List.fold (traverse_edge src u) l s)) =
     Q_list (queue s) @ filter (Not \<circ> discovered src (parent s)) l"
  using assms
...
next
  ...
  also have
    "... =
     Q_list (queue (traverse_edge src u v s)) @
     filter (Not \<circ> discovered src (parent (traverse_edge src u v s))) vs"
    using Cons.prems
  ...
  also have
    "... =
     Q_list (queue s) @
     (if \<not> discovered src (parent s) v then [v] else []) @
     filter (Not \<circ> discovered src (parent s)) vs"
    using Cons.prems(2, 3)
    by (simp add: list_queue_fold_cong_aux)
  also have "... = Q_list (queue s) @ filter (Not \<circ> discovered src (parent s)) (v # vs)"
    by simp
  finally show ?case
    .
qed
...
lemma list_queue_fold_cong_2:
  ...
  shows
    "Q_list (queue (fold G src s)) =
     ...
     filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s)))"
proof -
  have
    "Q_list (queue (fold G src s)) =
     ...
     filter
      ...
      (G.adjacency G (Q_head (queue s)))"
    using assms
    by (intro invar_tail G.distinct_adjacency list_queue_fold_cong) simp+
  thus ?thesis
...
proof
  ...
  show "v \<in> G.dV G"
  proof (cases "v \<in> set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))))")
    case True
    thus ?thesis
  ...
  shows
    "P_lookup (parent (List.fold (traverse_edge src u) l s)) =
     override_on
      (P_lookup (parent s))
      (\<lambda>_. Some u)
      (set (filter (Not \<circ> discovered src (parent s)) l))"
  using assms
proof (induct l arbitrary: s)
  case Nil
  thus ?case
  ...
  also have
    "... =
     override_on
      (P_lookup (parent (traverse_edge src u v s)))
      (\<lambda>_. Some u)
      (set (filter (Not \<circ> discovered src (parent (traverse_edge src u v s))) vs))"
    using Cons.prems
    by (fastforce intro: invar_parent_traverse_edge Cons.hyps)
  also have
    "... =
     override_on
      (override_on (P_lookup (parent s)) (\<lambda>_. Some u) (if \<not> discovered src (parent s) v then {v} else {}))
      (\<lambda>_. Some u)
      (set (filter (Not \<circ> discovered src (parent (traverse_edge src u v s))) vs))"
    using Cons.prems(1)
  ...
  also have
    "... =
     override_on
      (override_on (P_lookup (parent s)) (\<lambda>_. Some u) (if \<not> discovered src (parent s) v then {v} else {}))
      (\<lambda>_. Some u)
      (set (filter (Not \<circ> discovered src (parent s)) vs))"
    using Cons.prems
    by (simp add: list_queue_fold_cong_aux)
  finally show ?case
  ...
  shows
    "P_lookup (parent (fold G src s)) =
     override_on
      ...
      (set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s)))))"
proof -
  have
    "P_lookup (parent (fold G src s)) =
     override_on
      ...
      (\<lambda>_. Some (Q_head (queue s)))
      (set (filter (Not \<circ> discovered src (parent (s\<lparr>queue := Q_tail (queue s)\<rparr>))) (G.adjacency G (Q_head (queue s)))))"
    using assms
    by (intro G.distinct_adjacency lookup_parent_fold_cong) simp+
  thus ?thesis
    by (simp add: discovered_def)
qed
...
lemma invar_parent_fold_2:
  assumes "G.invar G"
  assumes "P_invar (parent s)"
  shows "P_invar (parent (fold G src s))"
  using assms
  by (intro G.distinct_adjacency invar_parent_fold) simp+

subsubsection \<open>@{term "P.dom \<circ> parent"}\<close>

lemma dom_parent_fold_subset_dV:
  assumes "P_invar (parent s)"
  assumes "distinct l"
...
proof
  ...
  show "v \<in> G.dV G"
  proof (cases "v \<in> set (filter (Not \<circ> discovered src (parent s)) l)")
    case True
    thus ?thesis
      using assms(4)
      by auto
...
lemma dom_parent_fold_subset_dV_2:
  assumes "G.invar G"
  assumes "P_invar (parent s)"
  assumes "P.dom (parent s) \<subseteq> G.dV G"
  shows "P.dom (parent (fold G src s)) \<subseteq> G.dV G"
  using assms G.adjacency_subset_dV
  by (intro G.distinct_adjacency dom_parent_fold_subset_dV) simp+
...
lemma ran_parent_fold_cong:
  ...
  shows
    "P.ran (parent (fold G src s)) =
     ...
     (if set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s)))) = {}
      then {}
      else {Q_head (queue s)})"
proof
  let ?S = "set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))))"
  ...
  proof
    ...
    proof (cases "u \<in> P.ran (parent s)")
      ...
      moreover hence "discovered src (parent s) v"
        by (simp add: discovered_def)
...
lemma T_fold_cong_aux:
  assumes "P_invar (parent s)"
  assumes "distinct (v # vs)"
  shows "w \<in> set vs \<and> \<not> discovered src (parent (traverse_edge src u v s)) w \<longleftrightarrow> w \<in> set vs \<and> \<not> discovered src (parent s) w"
  using assms
...
lemma T_fold_cong:
  assumes "P_invar (parent s)"
  assumes "distinct l"
  shows "T (parent (List.fold (traverse_edge src u) l s)) = T (parent s) \<union> {(u, v) |v. v \<in> set l \<and> \<not> discovered src (parent s) v}"
  using assms
proof (induct l arbitrary: s)
  case Nil
  thus ?case
...
next
  ...
  also have
    "... =
     ...
     {(u, w) |w. w \<in> set vs \<and> \<not> discovered src (parent (traverse_edge src u v s)) w}"
    using Cons.prems
    by (intro invar_parent_traverse_edge Cons.hyps) simp+
  also have
    "... =
     T (parent s) \<union>
     (if \<not> discovered src (parent s) v then {(u, v)} else {}) \<union>
     {(u, w) |w. w \<in> set vs \<and> \<not> discovered src (parent (traverse_edge src u v s)) w}"
  ...
  also have
    "... =
     T (parent s) \<union>
     (if \<not> discovered src (parent s) v then {(u, v)} else {}) \<union>
     {(u, w) |w. w \<in> set vs \<and> \<not> discovered src (parent s) w}"
    using Cons.prems
    by (simp add: T_fold_cong_aux)
  also have "... = T (parent s) \<union> {(u, w) |w. w \<in> set (v # vs) \<and> \<not> discovered src (parent s) w}"
    by force
  finally show ?case
    .
qed
...
lemma T_fold_cong_2:
  ...
  shows
    "T (parent (fold G src s)) =
     T (parent s) \<union>
     {(Q_head (queue s), v) |v. v \<in> set (G.adjacency G (Q_head (queue s))) \<and> \<not> discovered src (parent s) v}"
proof -
  have
    "T (parent (fold G src s)) =
     T (parent (s\<lparr>queue := Q_tail (queue s)\<rparr>)) \<union>
     {(Q_head (queue s), v) |v.
      v \<in> set (G.adjacency G (Q_head (queue s))) \<and>
      \<not> discovered src (parent (s\<lparr>queue := Q_tail (queue s)\<rparr>)) v}"
    using assms
    by (intro G.distinct_adjacency T_fold_cong) simp+
  thus ?thesis
    by (simp add: discovered_def)
qed
...
lemma loop_dom_aux:
  ...
  shows
    "card (P.dom (parent (fold G src s))) =
     card (P.dom (parent s)) +
     card (set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s)))))"
proof -
  let ?S = "set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))))"
...
proof (induct "card (G.dV G) + length (Q_list (queue s)) - card (P.dom (parent s))"
  ...
  next
    ...
    have "length (Q_list (queue (fold G src s))) = length (Q_list ?q) + card ?S"
      using less.prems(1-3) False G.distinct_adjacency
...
abbreviation (in bfs_invar_tbd) d :: "'m \<Rightarrow> 'a \<Rightarrow> nat" where
  "d m v \<equiv> dpath_length (rev_follow m v)"

abbreviation (in bfs_invar_tbd) white :: "('q, 'm) state \<Rightarrow> 'a \<Rightarrow> bool" where
  "white s v \<equiv> \<not> discovered src (parent s) v"

abbreviation (in bfs_invar_tbd) gray :: "('q, 'm) state \<Rightarrow> 'a \<Rightarrow> bool" where
  "gray s v \<equiv> discovered src (parent s) v \<and> v \<in> set (Q_list (queue s))"

abbreviation (in bfs_invar_tbd) black :: "('q, 'm) state \<Rightarrow> 'a \<Rightarrow> bool" where
...
locale bfs_invar =
  ...
  assumes d_triangle_inequality: "\<lbrakk> dpath_bet (G.dE G) p u v; \<not> white s u; \<not> white s v \<rbrakk> \<Longrightarrow> d (parent s) v \<le> d (parent s) u + dpath_length p"
...
abbreviation (in bfs_invar_tbd) bfs_invar_not_DONE'' :: "('q, 'm) state \<Rightarrow> bool" where
...
abbreviation (in bfs_invar_tbd) bfs_invar_DONE'' :: "('q, 'm) state \<Rightarrow> bool" where
...
subsubsection \<open>@{term bfs_invar_tbd}\<close>

context bfs_invar_tbd
begin
...
lemma (in bfs_invar_tbd) vertex_color_induct [case_names white gray black]:
...
lemma (in bfs_invar_tbd) src_not_white:
...
lemma (in bfs_invar_tbd) parent_imp_d:
...
proof (induct v rule: follow_pinduct)
  ...
  proof (cases "v = src")
    ...
    thus ?thesis
      ...
      by (auto simp add: follow_psimps intro: dpath_bet_reflexive)
  next
    case False
    then obtain u where
...
lemma (in bfs_invar_tbd) invar_queue_init:
...
lemma (in bfs_invar_tbd) invar_parent_init:
...
lemma (in bfs_invar_tbd) parent_imp_edge_init:
...
lemma (in bfs_invar_tbd) not_white_if_mem_queue_init:
...
lemma (in bfs_invar_tbd) not_white_if_parent_init:
  assumes "P_lookup (parent (init src)) v = Some u"
...
lemma (in bfs_invar_tbd) black_imp_adjacency_not_white_init:
...
lemma (in bfs_invar_tbd) queue_sorted_wrt_d_init:
...
lemma (in bfs_invar_tbd) d_last_queue_le_init:
...
lemma (in bfs_invar_tbd) d_triangle_inequality_init:
...
lemma (in bfs_invar_not_DONE) follow_invar'_parent_fold:
  shows "follow_invar' (P_lookup (parent (fold G src s)))"
  unfolding follow_invar'_def T_fold_cong
proof (rule wf_Un)
  let ?r =
    "{(Q_head (queue s), v) |v.
      v \<in> set (G.adjacency G (Q_head (queue s))) \<and> \<not> discovered src (parent s) v}"
  show "wf (T (parent s))"
    using follow_invar'
...
proof (standard, goal_cases)
  case 1
  show ?case
    by (intro follow_invar'_parent_fold)
qed
...
lemma (in bfs_invar_not_DONE) head_queueI:
  assumes "v \<in> set (Q_list (queue s))"
...
lemma (in bfs_invar) head_queueI_2:
...
lemma (in bfs_invar_not_DONE) whiteD:
  ...
  using assms
  by (auto simp add: discovered_def lookup_parent_fold_cong list_queue_fold_cong)
...
lemma (in bfs_invar_not_DONE) mem_filterD:
  assumes "v \<in> set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))))"
  shows
...
lemma (in bfs_invar) white_not_white_foldD:
  ...
  shows
    "v \<in> set (G.adjacency G (Q_head (queue s)))"
    "P_lookup (parent (fold G src s)) v = Some (Q_head (queue s))"
proof -
  show "v \<in> set (G.adjacency G (Q_head (queue s)))"
    using assms
...
next
  case False
  thus ?thesis
    using assms head_queueI_2 G.mem_adjacency_iff_edge
...
proof (cases "v \<in> set (Q_list (queue s))")
  case True
  thus ?thesis
    by (intro not_white_if_mem_queue not_whiteD(1))
next
  case False
  hence "v \<notin> set (Q_list (Q_tail (queue s)))"
    using invar_queue list_queue_non_empty
    by (auto simp add: Q.list_tail intro: list.set_sel(2))
  hence "v \<in> set (G.adjacency G (Q_head (queue s))) \<and> \<not> discovered src (parent s) v"
    using assms
    by (simp add: list_queue_fold_cong)
  thus ?thesis
    using assms
...
proof (cases "P_lookup (parent s) v = Some u")
  case True
  thus ?thesis
    by (intro not_white_if_parent not_whiteD(1))
next
  case False
...
next
  ...
  hence "u = Q_head (queue s)"
    using assms(1)
    by (intro head_queueI) simp+
  thus ?case
    using assms(2)
    by (simp add: discovered_def lookup_parent_fold_cong override_on_def G.mem_adjacency_iff_edge)
next
  case black
  thus ?case
    using assms(2)
    by (intro black_imp_adjacency_not_white not_whiteD(1))
qed
...
lemma (in bfs_invar_not_DONE) queue_sorted_wrt_d_fold_aux:
  ...
  assumes v_mem_filter: "v \<in> set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))))"
...
proof -
  ...
  moreover have "sorted_wrt ?P (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))))"
    by (auto simp add: mem_filterD(1) intro: sorted_wrt_if)
  moreover have
    "\<forall>u\<in>set (Q_list (Q_tail (queue s))).
      \<forall>v\<in>set (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s)))).
...
proof (cases "filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))) = []")
  case True
...
proof (cases "Q_list (Q_tail (queue s)) = []")
  ...
  hence
    "Q_head (queue (fold G src s)) =
     hd (filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))))"
    by (simp add: True list_queue_fold_cong)
  moreover have "filter (Not \<circ> discovered src (parent s)) (G.adjacency G (Q_head (queue s))) \<noteq> []"
    using invar_G invar_queue not_DONE invar_queue_fold_2 assms
...
proof (standard, goal_cases)
  case 1
  show ?case using follow_invar'_parent_fold .
next
  case 2
...
abbreviation (in bfs) shortest_dpath :: "'n \<Rightarrow> 'a list \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> bool" where
  "shortest_dpath G p u v \<equiv> dpath_bet (G.dE G) p u v \<and> dpath_length p = dist G u v"
...
lemma (in bfs_invar) queue_subset_dV:
  ...
  by (auto intro: dpath_bet_endpoints(2))
...
lemma (in bfs_invar_tbd) completeness:
  assumes "bfs_invar'' s"
  assumes "\<not> discovered src (parent (loop G src s)) v"
  shows "\<not> reachable (G.dE G) src v"
  using assms
...
proof -
  ...
  next
    ...
    next
      case False
      have shortest_dpath: "shortest_dpath G (l @ [u]) src u"
        using "3.prems"(1)
        by
          (auto
      ...
      also have "... \<le> dist G src u + 1"
        using "3.hyps"[OF shortest_dpath False]
...
lemma (in bfs_invar_DONE) not_white_imp_shortest_dpath:
  assumes "\<not> white s v"
  shows "shortest_dpath G (rev_follow (parent s) v) src v"
proof (rule conjI)
...
lemma (in bfs_invar_tbd) soundness:
  assumes "bfs_invar'' s"
  assumes "discovered src (parent (loop G src s)) v"
  shows "shortest_dpath G (rev_follow (parent (loop G src s)) v) src v"
  using assms
proof (induct rule: bfs_induct[OF assms(1)])
  case (1 G src s)
  show ?case
  proof (cases "DONE s")
    ...
    thus ?thesis
      using "1.prems"(2)
      by (intro bfs_invar_DONE.not_white_imp_shortest_dpath) (simp_all add: bfs_invar_DONE.loop_psimps)
  next
    case False
    with "1.prems"(1)
    have "bfs_invar_not_DONE' G src s"
...
abbreviation (in bfs) shortest_dpath_Map :: "'n \<Rightarrow> 'a \<Rightarrow> 'm \<Rightarrow> bool" where
  "shortest_dpath_Map G src m \<equiv>
   \<forall>v. (discovered src m v \<longrightarrow> shortest_dpath G (rev_follow m v) src v) \<and>
       (\<not> discovered src m v \<longrightarrow> \<not> reachable (G.dE G) src v)"

lemma (in bfs_invar_tbd) correctness:
  assumes "bfs_invar'' s"
  shows "shortest_dpath_Map G src (parent (loop G src s))"
