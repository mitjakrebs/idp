...
text \<open>This section considers maps from three levels of abstraction.\<close>

theory Map_Specs_Ext
  imports
    "../Misc_Ext"
...
text \<open>
On the high level, a map is a function (@{type map}). On the medium level, a map is specified via
the interfaces @{locale Map} and @{locale Map_by_Ordered}. We extend theory
...
lemma \<^marker>\<open>tag invisible\<close> (in Map_by_Ordered) set_filter_inorder_cong:
  assumes "invar m"
...
lemma \<^marker>\<open>tag invisible\<close> sorted1D:
...
lemma \<^marker>\<open>tag invisible\<close> set_del_list_cong:
  assumes "sorted1 l"
...
lemma \<^marker>\<open>tag invisible\<close> sorted1D_2:
  assumes "sorted (a # map fst ps)"
  assumes "x < a"
...
proof -
  have "map_of ps x = None"
    using assms
    by (intro map_of_sorted_Cons)
  thus ?thesis
    using assms
    by (auto simp add: map_of_eq_Some_iff_mem[symmetric] dest: sorted_cons)
qed

lemma \<^marker>\<open>tag invisible\<close> set_upd_list_cong:
  assumes "sorted1 l"
...
text \<open>We define the domain and range of a map.\<close>

definition (in Map) dom :: "'m \<Rightarrow> 'a set" where
  "dom m \<equiv> {a. lookup m a \<noteq> None}"

lemma \<^marker>\<open>tag invisible\<close> (in Map) mem_dom_iff:
...
lemma \<^marker>\<open>tag invisible\<close> (in Map) finite_dom_imp_finite_ran:
...
text \<open>
On the low level, the interfaces @{locale Map} and @{locale Map_by_Ordered} are implemented via
red-black trees.
\<close>
