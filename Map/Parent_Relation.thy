theory Parent_Relation
  imports
    Main
begin

text \<open>We (redefine and) extend the formalization of a well-formed parent relation.\<close>
...
lemma (in parent) follow_dom_if_wfP_follow_rel:
  assumes "wfP follow_rel"
  shows "follow_dom v"
  using assms
  by (intro accp_wfPD)

lemma (in parent) follow_dom_if_wf_follow_rel:
...
lemma (in parent) parent_last_follow_eq_None:
