...
abbreviation (in alt_bfs) alt_bfs_valid_input' :: "'n \<Rightarrow> 'n \<Rightarrow> 'a \<Rightarrow> bool" where
...
abbreviation (in alt_bfs_valid_input) alt_bfs_invar'' :: "('q, 'm) state \<Rightarrow> bool" where
...
abbreviation (in alt_bfs_valid_input) alt_bfs_invar_not_DONE'' :: "('q, 'm) state \<Rightarrow> bool" where
...
abbreviation (in alt_bfs_valid_input) alt_bfs_invar_DONE'' :: "('q, 'm) state \<Rightarrow> bool" where
...
subsubsection \<open>@{term alt_bfs_valid_input}\<close>

lemma (in alt_bfs_valid_input) vertex_color_induct [case_names white gray black]:
...
lemma (in alt_bfs_valid_input) Q_P''_cong:
...
lemma (in alt_bfs_valid_input) Q_P''_cong_2:
...
lemma (in alt_bfs_valid_input) mem_E_if_mem_E1:
  assumes "e \<in> G.E G1"
  shows "e \<in> G.E G"
  using invar_G1 invar_G2 assms
  by (simp add: G.E_union_cong)

lemma (in alt_bfs_valid_input) mem_E_if_mem_E2:
  assumes "e \<in> G.E G2"
  shows "e \<in> G.E G"
  using invar_G1 invar_G2 assms
  by (simp add: G.E_union_cong)

lemma (in alt_bfs_valid_input) mem_E1_iff_not_mem_E2:
  assumes "e \<in> G.E G"
...
lemma (in alt_bfs_valid_input) src_mem_V:
  shows "src \<in> G.V G"
...
lemma (in alt_bfs_valid_input) mem_adjacency_imp_edge:
  assumes "v \<in> set (adjacency G1 G2 s u)"
  shows "{u, v} \<in> G.E G"
  using assms
  by
...
lemma (in alt_bfs_valid_input) mem_adjacency_if_edge:
  assumes "alt s u v"
...
lemma (in alt_bfs_valid_input) src_not_white:
...
lemma (in alt_bfs_valid_input) P'E:
  assumes "P' G2 (P_lookup (parent s) v) v"
  obtains u where
...
lemma (in alt_bfs_valid_input) follow_invar_parent_init:
...
lemma (in alt_bfs_valid_input) invar_queue_init:
...
lemma (in alt_bfs_valid_input) invar_parent_init:
...
lemma (in alt_bfs_valid_input) parent_src_init:
...
lemma (in alt_bfs_valid_input) parent_imp_alt_init:
...
lemma (in alt_bfs_valid_input) parent_imp_edge_init:
  assumes "P_lookup (parent (init src)) v = Some u"
  shows "{u, v} \<in> G.E G"
  using assms
  by (simp add: init_def P.map_empty)
...
lemma (in alt_bfs_valid_input) not_white_if_mem_queue_init:
...
lemma (in alt_bfs_valid_input) not_white_if_parent_init:
  assumes "P_lookup (parent (init src)) v = Some u"
...
lemma (in alt_bfs_valid_input) black_imp_adjacency_not_white_init:
...
lemma (in alt_bfs_valid_input) queue_sorted_wrt_d_init:
...
lemma (in alt_bfs_valid_input) d_last_queue_le_init:
...
lemma (in alt_bfs_valid_input) d_triangle_inequality_init:
...
lemma (in alt_bfs_valid_input) alt_bfs_invar_alt_loop:
  assumes "alt_bfs_invar'' s"
  shows "alt_bfs_invar'' (alt_loop G1 G2 src s)"
  using assms
  by (intro alt_bfs_invar.alt_bfs_invar_alt_loop)

lemma (in alt_bfs_valid_input) alt_bfs_invar_alt_loop_init:
...
lemma (in alt_bfs_valid_input) completeness:
  assumes "alt_bfs_invar'' s"
...
lemma (in alt_bfs_valid_input) soundness:
  assumes "alt_bfs_invar'' s"
...
theorem (in alt_bfs_valid_input) alt_bfs_correct:
